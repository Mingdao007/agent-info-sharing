...
| `robotics.rnn_jacobian_force_motion_control` | non_ros, ros2 | current_implementation_baseline, framework_logic_reference, offline_analysis | index_only, pullable_if_future_cache_exists | `damped least squares`, `dls`, `dls solver`, `frame sign`, `frame-sign`, `finite time convergence`, `force motion control`, `inverse kinematics` | Pinocchio; OSQP; qpsolvers; Task Space Inverse Dynamics | Robotics theory/implementation source starting point. Use these for kinematics, QP, and task-space control references; paper-specific RNN, finite-time convergence, or force-control claims still require source-grounded literature review. |
